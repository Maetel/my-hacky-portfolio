...
import VirtualWidget from "./Widget";
...
export default class WidgetManager {
  ...
  static _widgets: VirtualWidget[] = [];
  static get widgets() {
    return WidgetManager._widgets;
  }
  static set widgets(widgets: VirtualWidget[]) {
    WidgetManager._widgets = widgets;
  }

  static push(...args: VirtualWidget[]) {
  ...
  static _push(widget: VirtualWidget) {
  ...
  static of(x: number, y: number): VirtualWidget | null {
  ...
  static id(id: string): VirtualWidget | null {
    return WidgetManager._widgets.find((widget) => widget.id === id) ?? null;
  }

  static pop(): VirtualWidget | null {
    return WidgetManager._widgets.pop() ?? null;
  }

  static async remove(id: string): Promise<VirtualWidget | null> {
...
async function deleteWidget(
  widget: VirtualWidget | string
): Promise<VirtualWidget | null> {
  return new Promise((resolve, reject) => {
    const predicate =
      typeof widget === "string"
        ? (w: VirtualWidget) => w.id === widget
        : (w: VirtualWidget) => w.id !== widget.id;
...
function removeWidgetFromList(widget: VirtualWidget) {
